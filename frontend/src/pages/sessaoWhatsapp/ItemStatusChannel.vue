    ...
    <q-item>
      <q-item-section avatar>
        <q-icon v-if="item.status == 'qrcode'"
          ...
          name="mdi-crop-free"
          size="2.5em" />
        <q-icon v-if="['DISCONNECTED', 'INSTAGRAM_BAD_PASSWORD'].includes(item.status)"
          color="negative"
          size="2.5em"
          name="mdi-wifi-alert" />
      ...
      <q-item-section>
        ...
        <q-item-label v-if="item.status == 'INSTAGRAM_BAD_PASSWORD'">
          <span class="text-weight-medium"> Login recusado pelo Instagram. </span>
          <span class="row col"> O Instagram solicitou a recuperação de acesso por e-mail. Confirme a senha entrando
            diretamente no Instagram e conclua a validação da conta antes de tentar conectar novamente.</span>
        </q-item-label>
